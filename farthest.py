...
class Farthest():
    ...
    def snap(self, centers):
        TEMPFILE = f"farthest{self.timestamp}.png"
        # fig, ax = plt.subplots()
    ...
    def initialize(self):
       ...
       centers = [self.data[np.random.randint(n_samples)]]

        # Step 2: Choose the remaining centers
       for _ in range(1, self.k):
            # Compute the distance from each point to its nearest center
            distances = np.array([min(self.dist(x, center) for center in centers) for x in self.data])
            
            # Choose the point farthest from the nearest center
            next_center_idx = np.argmax(distances)
            centers.append(self.data[next_center_idx])
